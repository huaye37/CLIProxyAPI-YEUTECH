...
def main():
    ...
    def probe(index):
        ...
        try:
            ...
            with urllib.request.urlopen(request, timeout=45 if args.model else 8) as response:
    ...
    success = sum(r.get('completed', False) if args.model else r.get('status') == 401 for r in results)
