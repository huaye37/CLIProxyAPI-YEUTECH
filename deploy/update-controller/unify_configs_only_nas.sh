...
STAGE=$(ssh yeutech-nas 'mktemp -d /tmp/yeutech-config-unify-XXXXXX')
...
scp -O "$ROOT/unify_configs_only.py" "yeutech-nas:$STAGE/"
...
test -n "$NAS_PASSWORD"
printf '%s\n' "$NAS_PASSWORD" | ssh yeutech-nas "sudo -S -p '' /usr/bin/python3 '$STAGE/unify_configs_only.py'"
