...
MANAGER="$ROOT/../../../yeutech-api-platform-redesign-20260914/public"
PASSWORD_FILE=/Users/fangjialiang/Documents/家庭网络中枢项目/04_工具与运维/工具脚本与网络配置/服务器密码.rtf
STAGE=$(ssh yeutech-nas 'mktemp -d /tmp/yeutech-merge-status-XXXXXX')
...
scp -O "$ROOT/agent.py" "$ROOT/release_merge_status.py" "$MANAGER/app.js" "$MANAGER/index.html" "$MANAGER/style.css" "yeutech-nas:$STAGE/"
...
test -n "$NAS_PASSWORD"
printf '%s\n' "$NAS_PASSWORD" | ssh yeutech-nas "sudo -S -p '' /usr/bin/python3 '$STAGE/release_merge_status.py' '$STAGE'"
